...
Shader "KriptoFX/ME/DistortionSlimeMobile" {
	Properties{
		...
		Category{
			...
			SubShader {
				Pass {
					...
					float _BumpAmt;
					sampler2D _ColorPyramidTextureMobile;
					float4 _ColorPyramidTextureMobile_TexelSize;
					...
					fixed4 frag(v2f i) : COLOR
					{
						...
						half2 offset = normal.rg * _BumpAmt * _ColorPyramidTextureMobile_TexelSize.xy * i.color.a;
						i.grab.xy = offset * i.grab.z + i.grab.xy;
						half4 col = tex2Dproj(_ColorPyramidTextureMobile, UNITY_PROJ_COORD(i.grab));
